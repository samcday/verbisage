//! Transplanted prefix + single-edit current-word ranking.
//!
//! Byte-for-byte logic move of the former `src/completion.rs` free function.
...
/// [`CompletionEngine`] wrapper around the transplanted [`complete()`].
///
/// Structural fitting only: delegates to the frozen free function. `None`
    ...
    fn complete(&self, prefix: Option<&str>, max: usize) -> Vec<CompletionCandidate> {
        ...
        complete(self.backend, input, max)
            ...
            .map(|r| {
                ...
                // weight 1.0 and edit corrections lower weights; the weight
                // itself is not returned, so reconstruct membership with the
                // same prefix test the query used.
                let is_exact = r.word.to_lowercase().starts_with(&lowered);
                CompletionCandidate {
                    word: r.word,
                    score: r.confidence,
                    is_exact,
